...
public class OptionTwo {
	public void optionTwo(ArrayList<HurricaneEntry> hurricanelist) throws IOException {
		String option;
		...
		switch(option) {
		case "1":
			res = sortByCategoryAscending(hurricanelist);
			displayOutput(res,1,"Category");
			writeOutput(res,1,"Category");
			break;
		case "2":
			res = sortByCategoryDescending(hurricanelist);
			displayOutput(res,2,"Category");
			writeOutput(res,2,"Category");
			break;
		default:
			JOptionPane.showMessageDialog(null, "Wrong Choice!");
			optionTwo(hurricanelist);
		}
	}//End of Option Two
	
	public static ArrayList<HurricaneEntry> sortByCategoryAscending(ArrayList<HurricaneEntry> hurricanelist) {
		ArrayList<HurricaneEntry> result  = new ArrayList<>();
		for(int j =1;j<=5;j++) {
			for(int k =0; k<hurricanelist.size();k++) {
				if(j == hurricanelist.get(k).getCategory()) {
		...
		return result;
	}// End of sortByCategoryAscending
	
	public static ArrayList<HurricaneEntry> sortByCategoryDescending(ArrayList<HurricaneEntry> hurricanelist) {
		ArrayList<HurricaneEntry> result  = new ArrayList<>();
		for(int j =5;j>=0;j--) {
			for(int k =0; k<hurricanelist.size();k++) {
				if(j == hurricanelist.get(k).getCategory()) {
		...
		return result;
	}// End of sortByCategoryDescending
