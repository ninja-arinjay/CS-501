...
public class OptionFour {
	public void optionFour(ArrayList<HurricaneEntry> hurricanelist) throws IOException{
		String option;
		...
		switch(option) {
		case "1":
			res = sortByMonthAscending(hurricanelist);
			displayOutput(res,1, "Month");
			writeOutput(res,1,"Month");
			break;
		case "2":
			res = sortByMonthDescending(hurricanelist);
			displayOutput(res,2, "Month");
			writeOutput(res,2,"Month");
			break;
		default:
			JOptionPane.showMessageDialog(null, "Wrong Choice!");
			optionFour(hurricanelist);
		}
	}//End of Option Four
	
	public static ArrayList<HurricaneEntry> sortByMonthAscending(ArrayList<HurricaneEntry> hurricanelist) {
		ArrayList<HurricaneEntry> result  = new ArrayList<>();
		for(int j =1;j<=12;j++) {
			for(int k =0; k<hurricanelist.size();k++) {
				if(j == hurricanelist.get(k).getMonth()) {
		...
		return result;
	}// End of sortByMonthAescending
	
	public static ArrayList<HurricaneEntry> sortByMonthDescending(ArrayList<HurricaneEntry> hurricanelist) {
		ArrayList<HurricaneEntry> result  = new ArrayList<>();
		for(int j =12;j>=1;j--) {
			for(int k =0; k<hurricanelist.size();k++) {
				if(j == hurricanelist.get(k).getMonth()) {
